...
# 主函数，包含直方图均衡化
def process_image_and_plot_histogramss(image_path):
    # 读取图像，并转换为灰度图像
    img = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)
    ...
    # 进行直方图均衡化
    equalized_img = equalize_hist_image(img)
    ...
    low_contrast_hist_img = calc_histogram_image(low_contrast_img)
    high_contrast_hist_img = calc_histogram_image(high_contrast_img)
    equalized_hist_img = calc_histogram_image(equalized_img)
...
if num == '4':
    image = 'mys.jpg'
    process_image_and_plot_histogramss(image)